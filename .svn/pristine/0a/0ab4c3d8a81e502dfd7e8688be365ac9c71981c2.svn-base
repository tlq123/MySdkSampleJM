...
奇梦 图灵的结合板测试
 */
public class DemoActivity extends Activity implements MQTTReceiveInterface {
    ...
    private boolean isWakeUpExit = true;
    /**
     * 当前返回的语义解析结果
     */
        ...
        MediaMusicUtil.getInstance().initMusic(this, new MediaMusicUtil.ImusicListener() {
            ...
            public void OnMusicCompletion() {
                ...
                if( 1 == mediaType ){
                    ...
                    if( !isAsrFlag ){
                        DemoBase.getInstance().doPost("下一个");
                    }
                }
            }
        ...
        public void onSuccess() {
            ...
            runOnUiThread(new Runnable() {
                ...
                public void run() {
                    ...
                    DemoBase.getInstance().getAll(httpClientListener,itsCallback,mContext);
                    initQM();
                    ...
                    MQTTBindManager.setOnMQTTReceiveListener(DemoActivity.this);
                }
            });
        }
        @Override
    ...
    private void processAsr(String content) {
        ...
        DemoBase.getInstance().doPost(discernASR);
    }
    ...
    */
    ...
    private OnHttpRequestListener httpClientListener = new OnHttpRequestListener() {
        ...
        public void onSuccess(final String s) {
            ...
            if (!TextUtils.isEmpty(s)) {
                ...
                setAsrFlag(true);
                Logger.d(Logger.TAG, "httpClientListener|onSuccess|response:" + getString(R.string.str_response_null));
            }
        }
    ...
    private TTSListener itsCallback = new TTSListener() {
        ...
        public void onSpeakCompleted() {
            ...
            if (behavior != null && ( isMusic(behavior.getIntent()) || isMedia(behavior.getIntent()) ) ) {
...
//             saveWavData(data);
    ...
    private void processSequence(final String content) {
        ...
        Logger.d(TAG,"processSequence curentBehavior:"+behavior.toString());
        ...
        if( isMedia(behavior.getIntent())  || isMusic(behavior.getIntent()) ){
    ...
    private boolean isMedia(Behavior.IntentInfo bean) {
    ...
    private void checkMusic(final Behavior.IntentInfo bean) {
        ...
        if (parameters == null) {
            Logger.d(TAG, "checkMusic parameters is null");
            return;
        }
    ...
    private BroadcastReceiver mtouchReceiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (ConnectivityManager.CONNECTIVITY_ACTION.equals(action)) {//网络变化
                ...
                DemoBase.getInstance().setPowerValue(powerValue);
    ...
    private long connNetAPTime = 0;
    private long interruptTime = 0;

    @Override
    public boolean onKeyDown(int keyCode, KeyEvent event) {
        Logger.i(TAG, "keyCode:" + keyCode);
        if (keyCode == KeyEvent.KEYCODE_BACK || keyCode == 79 || keyCode == 10) {
            if (event.getRepeatCount() == 0){
                downTime = System.currentTimeMillis();
            }
            if(MyNetUtil.isNetworkAvailable(mContext)){
                if((System.currentTimeMillis() - interruptTime) > 3000){//防止重复点击
                    interruptTime = System.currentTimeMillis();
                    isWakeUpExit = true ;
                    exitLogic();
                }
            }else {
                if((System.currentTimeMillis() - connNetAPTime) > 5000){//防止重复点击
                    connNetAPTime = System.currentTimeMillis();
    ...
    public boolean onKeyUp(int keyCode, KeyEvent event) {
        if(System.currentTimeMillis()-downTime > 10*1000){
            ConfigureNetworkUtil.connNetAP(mContext);
        }
